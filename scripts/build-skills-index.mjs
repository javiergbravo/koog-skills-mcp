#!/usr/bin/env node
// Walks ../skills (or --skills-dir), parses each SKILL.md via @koog-skills/core,
// writes a JSON snapshot to --out for bundling into the mcp package.
...
// Resolve @koog-skills/core via its built dist file rather than node module
...
if (!values.out) {
	console.error(
		"Usage: build-skills-index.mjs --out <path> [--skills-dir <path>]",
	);
	process.exit(2);
}
...
const portable = skills.map((s) => ({
	...
	path: s.path.startsWith(skillsRootResolved)
		...
		: s.path,
	dir: s.dir.startsWith(skillsRootResolved)
		? s.dir.slice(skillsRootResolved.length + 1)
		: s.dir,
