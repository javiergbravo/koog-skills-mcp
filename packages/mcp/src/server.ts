import { buildIndex, type Skill, type SkillIndex } from "@koog-skills/core";
import {
	McpServer,
	ResourceTemplate,
...
function registerGetSkill(server: McpServer, byName: Map<string, Skill>): void {
	server.registerTool(
		"get_skill",
		{
			...
			inputSchema: {
				name: z
					.string()
					.min(1)
					.describe('Skill name (kebab-case, e.g. "graph-based-agents").'),
				include_references: z
					.boolean()
					.optional()
					.describe(
						"If true, inline all referenced files at the end. Default false.",
					),
			},
		},
		async ({ name, include_references }) => {
			...
			const text = rebuildSkillMarkdown(skill, {
				includeReferences: include_references ?? false,
			});
