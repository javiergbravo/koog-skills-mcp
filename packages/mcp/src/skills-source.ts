...
import { type Skill, loadSkills } from '@koog-skills/core';
import { z } from 'zod';
...
const SkillReferenceSchema = z.object({
  relPath: z.string(),
  absPath: z.string().regex(REL_PATH_RE, 'absPath in bundle must be a relative path without traversal'),
  content: z.string().optional(),
});

const SkillFrontmatterBundleSchema = z.object({
  name: z.string(),
  description: z.string(),
  license: z.string().optional(),
  compatibility: z.string().optional(),
  allowedTools: z.string().optional(),
  metadata: z.object({ author: z.string().optional(), version: z.string().optional(), keywords: z.array(z.string()) }).passthrough().default({ keywords: [] }),
}).passthrough();
...
const SkillBundleSchema = z.array(
  z.object({
    ...
    description: z.string(),
    category: z.string(),
    path: z.string().regex(REL_PATH_RE, 'path in bundle must be a relative path without traversal'),
    dir: z.string().regex(REL_PATH_RE, 'dir in bundle must be a relative path without traversal'),
    body: z.string(),
...
export async function loadSkillsForServer(opts: LoadOpts = {}): Promise<Skill[]> {
  ...
  try {
    ...
    throw new Error(`Failed to parse skills bundle at ${bundlePath}: invalid JSON`);
  }
